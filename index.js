...
// Middleware
app.use(cors({ origin: "*" })); // Adjust for your frontend URL in production
...
async function startApp() {
  try {
    ...
    console.log("App initialized with MongoDB");
  } catch (err) {
...
const verifyAdmin = async (req, res, next) => {
  try {
    ...
    console.error("verifyAdmin error:", error.message, error.stack);
...
const restrictDuplicateReview = async (req, res, next) => {
  try {
    ...
    console.error("restrictDuplicateReview error:", err.message, err.stack);
    res.status(500).json({ message: "Server error" });
  }
};
...
const restrictDuplicateRequest = async (req, res, next) => {
  try {
    ...
    console.error("restrictDuplicateRequest error:", err.message, err.stack);
    res.status(500).json({ message: "Server error" });
  }
};
...
app.get("/users/admin/:email", verifyJWT, async (req, res) => {
  try {
    ...
    console.error("Error checking admin status:", err.message, err.stack);
    res.status(500).json({ message: "Server error" });
  }
});
...
app.patch("/users/admin/:id", verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    console.error("Error updating admin role:", err.message, err.stack);
    res.status(500).json({ message: "Server error" });
  }
});
...
app.get("/meals", async (req, res) => {
  try {
    ...
    console.log("Fetching meals with query:", req.query);
    ...
    console.error("Error fetching meals:", err.message, err.stack);
...
app.post("/meals", verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    console.error("Error creating meal:", err.message, err.stack);
    res.status(500).json({ message: "Server error" });
  }
});
...
app.get("/meals/stats", verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    console.log("Valid meal IDs:", { count: validMealIds.length });
    ...
    console.error("Error fetching meal stats:", err.message, err.stack);
...
app.get("/meals/unserved", verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    console.log(`Fetched ${unserved.length} unserved meals`);
    ...
    console.error("Error fetching unserved meals:", err.message, err.stack);
    if (err.name === "BSONError") {
      return res.status(400).json({ message: "Invalid meal ID format in database" });
    }
    res.status(500).json({ message: "Server error" });
...
app.get("/meals/:id", async (req, res) => {
  try {
    ...
    console.error("Error fetching meal:", err.message, err.stack);
    if (err.name === "BSONError") {
      return res.status(400).json({ message: "Invalid meal ID format" });
    }
    res.status(500).json({ message: "Server error" });
...
app.patch("/meals/like/:id", verifyJWT, async (req, res) => {
  try {
    ...
    console.log(`Like ${alreadyLiked ? "removed" : "added"} for mealId: ${mealId}`);
    res.json({ success: true, modifiedCount: result.modifiedCount });
  } catch (err) {
    console.error("Error liking meal:", err.message, err.stack);
    if (err.name === "BSONError") {
      return res.status(400).json({ message: "Invalid meal ID format" });
    }
    res.status(500).json({ message: "Server error" });
...
app.get("/my-likes", verifyJWT, async (req, res) => {
  try {
    ...
    console.error("Error fetching liked meals:", err.message, err.stack);
    res.status(500).json({ message: "Server error" });
  }
});
...
app.get("/admin/meals", verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    console.error("Error fetching admin meals:", err.message, err.stack);
    res.status(500).json({ message: "Server error" });
  }
});
...
app.delete("/meals/:id", verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    console.error("Error deleting meal:", err.message, err.stack);
    if (err.name === "BSONError") {
      return res.status(400).json({ message: "Invalid meal ID format" });
    }
    res.status(500).json({ message: "Server error" });
...
app.patch("/meals/:id", verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    console.error("Error updating meal:", err.message, err.stack);
...
app.post("/meal-request", verifyJWT, restrictDuplicateRequest, async (req, res) => {
  try {
    ...
    const { mealId } = req.body;
    const userEmail = req.user.email;
    console.log(`🛰️ [POST] /meal-request`, { mealId, userEmail });
    ...
    console.log(`Meal request created: ${result.insertedId}`);
    res.status(201).json({ success: true, insertedId: result.insertedId });
  } catch (err) {
    console.error("Error requesting meal:", err.message, err.stack);
    if (err.name === "BSONError") {
      return res.status(400).json({ success: false, message: "Invalid meal ID format" });
    }
...
app.get("/requested-meals", verifyJWT, async (req, res) => {
  try {
    ...
    console.log(`Fetched ${detailedRequests.length} requested meals for ${userEmail}`);
    res.json({
    ...
    console.error("Error fetching requested meals:", err.message, err.stack);
    res.status(500).json({ success: false, message: "Server error" });
  }
});
...
app.delete("/requested-meals/:id", verifyJWT, async (req, res) => {
  try {
    ...
    console.error("Error deleting request:", err.message, err.stack);
    if (err.name === "BSONError") {
      return res.status(400).json({ success: false, message: "Invalid request ID" });
    }
    res.status(500).json({ success: false, message: "Server error" });
...
app.patch("/meals/serve/:id", verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    console.log(`Order served: ${orderId}`);
    res.json({ success: true, modifiedCount: result.modifiedCount });
  } catch (err) {
    console.error("Error serving meal:", err.message, err.stack);
    if (err.name === "BSONError") {
...
app.post("/upcoming-meals", verifyJWT, verifyAdmin, async (req, res) => {
  try {
    ...
    console.error("Error adding upcoming meal:", err.message, err.stack);
    res.status(500).json({ success: false, message: "Server error" });
  }
});
...
app.get("/upcoming-meals", async (req, res) => {
  try {
    ...
    console.error("Error fetching upcoming meals:", err.message, err.stack);
    res.status(500).json({ success: false, message: "Server error" });
  }
});
...
app.patch("/upcoming-meals/like/:id", verifyJWT, async (req, res) => {
  try {
    ...
    console.error("Error liking upcoming meal:", err.message, err.stack);
    if (err.name === "BSONError") {
      return res.status(400).json({ message: "Invalid meal ID format" });
    }
    res.status(500).json({ message: "Server error" });
...
// Packages (Fixed: Define packageDetails)
...
app.post("/confirm-payment", verifyJWT, async (req, res) => {
  try {
    ...
    console.log("Received /confirm-payment:", { packageName, transactionId, userEmail });
    ...
    console.log(`Payment recorded: ${result.insertedId}, badge updated for ${userEmail}: ${packageName}`);
...
app.get("/payments", verifyJWT, async (req, res) => {
  try {
    ...
    console.log(`Fetched ${payments.length} payments for ${userEmail}`);
    res.json({
      payments,
      total,
      page: 1,
    ...
    console.error("Error fetching payments:", err.message, err.stack);
    res.status(500).json({ message: "Server error", error: err.message });
  }
});
...
app.get("/my-reviews", verifyJWT, async (req, res) => {
  try {
    ...
    console.log(`Fetched ${detailedReviews.length} reviews for ${userEmail}`);
    res.json({
    ...
    console.error("Error fetching user reviews:", err.message, err.stack);
    res.status(500).json({ message: "Server error", error: err.message });
  }
});
